...
4. `V4`
    * Функции:
        ...
        2. `addAdmin(address addr, string username, string fullName, bytes32 secretHash)` - добавить в систему нового администратора
            * **Разрешенные роли**:
            ...
            * **Принимает**:
                ...
                4. `bytes32 secretHash` - хеш второго пароля нового администратора (получается через `web3.utils.keccak256(string)`)
5. `V5`
    * Структуры
